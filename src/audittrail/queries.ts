...
export async function addActivity(user_id: number, activity_name: String, cred_id: number | null = null) {
    // Input validation
    if (!user_id) {
        return {
            success: false,
    ...
    try {
        ...
        console.log('Audit Log Added Succesfully:', response.data)
    } catch (error) {
...
export async function getActivitiesLogs(user_id: number) {
    // Input validation
    if (!user_id) {
        return {
            success: false,
    ...
    try {
        ...
        // Fetch data from API with proper headers
        const response = await axios.get(`/api/audit-trail/${user_id}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest', // Helps with CSRF protection
            },
            timeout: 10000, // 10 second timeout
        ...
        console.log('Audit Trail Received:', response.data)

        const auditLogs = response.data.data?.logs || null;
