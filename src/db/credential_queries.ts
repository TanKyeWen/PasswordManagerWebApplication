...
export async function getIndividualCredential(user_id: number, credential_id: number) {
    try {
        ...
        const validateUser = await executeQuery(`
 ...
 */
export async function updateCredential(user_id: number, credential_id: number, credential: object) {
    // Input validation
    if (!user_id || typeof user_id !== 'number' || user_id <= 0) {
        return {
            success: false,
    ...
    try {
        ...
        // Fetch data from API with proper headers
        const response = await axios.put(`/api/credential/${credential_id}`, {
            user_id: user_id,
            ...credential
        }, {
            headers: {
        ...
        try {
            ...
            const updateSQL = `
                UPDATE credentials
                SET credential_website = ?, credential_username = ?, credential_password = ?
            ...
            console.log(`Credential ${credential_id} added for user ${user_id}`); 
...
export async function deleteIndividualCredential(user_id: number, credential_id: number) {
    try {
        ...
        if (parseInt(validateUser.result?.resultRows[0][0]) !== user_id) {
            return {
                success: false,
                error: 'Does not belong to the user',
                code: 401
            };
        }
